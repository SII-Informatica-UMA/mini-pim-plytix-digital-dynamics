...
public interface RelacionRepository extends JpaRepository<Relacion, Long> {
	@Query("SELECT r FROM Relaciones r WHERE r.id = :id")
	List<Relacion> findById(Integer id);
	@Query("INSERT INTO Relaciones r (r.nombre, r.descripcion) VALUES (:nombre, :descripcion)")
	Relacion insert(Relacion r);
	@Query("UPDATE Relaciones r SET r.nombre = :nombre, r.descripcion = :descripcion WHERE r.id = :id")
	Relacion modifyById(Relacion r);
	@Query("DELETE FROM Relaciones r WHERE r.id = :id")
